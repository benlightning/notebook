```php
        ...
        $lightface = dirname(__FILE__) . "/PingFangExtraLight.ttf";
        $heavyface = dirname(__FILE__) . "/PingFangHeavy.ttf";

        $topinfo = getimagesize(dirname(__FILE__) . "/images/top.png");//return array(0,1),0宽，1高
        $bottominfo = getimagesize(dirname(__FILE__) . "/images/bottom.png");
        $textinfo = getimagesize(dirname(__FILE__) . "/images/textbg.png");
        
        $imtop = imagecreatefrompng(dirname(__FILE__) . "/images/top.png");
        $imbottom = imagecreatefrompng(dirname(__FILE__) . "/images/bottom.png");
        $imtextbg = imagecreatefrompng(dirname(__FILE__) . "/images/textbg.png");
        $imleft = imagecreatefrompng(dirname(__FILE__) . "/images/left.png");
        $imright = imagecreatefrompng(dirname(__FILE__) . "/images/right.png");
        
        $name = str_replace('-', '', $date);
        $attach = MODULE_ROOT . '/images/' . $name . '.png';
        $attachUrl = MODULE_URL . 'images/' . $name . '.png';
        
        $width = 640;
        $height = ($textinfo[1]+10)*count($boxdata)+$topinfo[1]+$bottominfo[1]+40;
        $im = @imagecreatetruecolor($width, $height) or die('Cannot Initialize new GD image stream');
        $bgcolor = imagecolorallocate($im, 219,245,255);
        imagefill($im, 0, 0, $bgcolor);
//        $rectcolor = imagecolorallocate($im, 235,210,173);
//        imagefilledrectangle ( $im, 10 , 10 , $width-10 , $height-10 , $rectcolor );
        imagecopy($im, $imtop, 0, 0, 0, 0, $topinfo[0], $topinfo[1]);
        $top = $topinfo[1]+47;
        $y1 = $topinfo[1]+20;
        
        $textbgcolor = imagecolorallocate($im, 38,191,248);
        $fontcolor = imagecolorallocate($im, 0, 0, 0);
        $fontcolorFFF = imagecolorallocate($im, 255, 255, 255);
        
        imagettftext($im, 110, 0, 40, 270, $fontcolorFFF, $lightface, date('n.d',$time));
        imagettftext($im, 30, 0, 370, 260, $fontcolorFFF, $heavyface, $this->getweek($time));
        
        $lunar = new Lunar();
        $themonth = $lunar->convertSolarToLunar(date('Y',$time), date('n',$time), date('j',$time));//将阳历转换为阴历 
        imagettftext($im, 16, 0, 56, 332, $fontcolorFFF, $heavyface, $zhdate.' - '.$themonth[3].'年 - '.$themonth[1].' - '.$themonth[2]);
```
